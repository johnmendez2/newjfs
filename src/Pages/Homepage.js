...
function Homepage() {
  ...
  <div className='squareboxforfeatured' style={{padding:'20px'}}>
    <div class="square">
    <Link to={`/${product.id}-${product.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}>
      <img class="square-content" src={product.thumbnailImages[0]} alt='featuredimage' onClick={() => {handleClick(product.id)}} />
      </Link>
    </div>
  </div>
))}
